  ...
  return (
    <>
      ...
      <div className="container" id="buttonTF">
        ...
        <button className="btn btn-primary ml-1 my-2" onClick={handleLowerCase}>Convert to Lowercase</button>
        <button className="btn btn-primary" id="copy" onClick={handleCopy}>Copy text</button>
        <button className="btn btn-primary" id="clear" onClick={handleClear}>Clear text</button>
      </div>
      <div className={`container my-5 text-${props.mode === 'dark' ? 'light' : 'dark'}`} id="wordcount">
        <h2>WORD COUNT</h2>
        <p>NUmber of words: {text.split(/\s+/).length}</p>                <p>Number of letters: {text.length}</p>
        <h3>{0.008 * text.split(/\s+/).length} mins to read</h3>
      </div>
    </>
  );
}
